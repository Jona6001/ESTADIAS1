...
const Residuos = () => {
  ...
  const handleCrearResiduo = async (e) => {
    ...
    try {
      ...
      if (!res.ok || data?.success === false)
        throw new Error(data?.message || "Error al crear residuo");
      setForm({
        cotizacionId: "",
        productoId: "",
        m2_residuo: "",
  ...
  const handleEliminarResiduo = async (id) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`${API_RESIDUOS}/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
          Accept: "application/json",
      ...
      >
        ...
        >
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            <label style={{ fontSize: ".75rem", fontWeight: 600 }}>
              Cotización
            </label>
            <select
              className="user-input"
              value={form.cotizacionId}
              onChange={(e) => handleFormChange("cotizacionId", e.target.value)}
            >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            ...
            <input
              ...
              onChange={(e) => handleFormChange("m2_residuo", e.target.value)}
            />
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            <label style={{ fontSize: ".75rem", fontWeight: 600 }}>
              ...
              {loading ? (
                ...
                filtrados.map((r) => (
                  <tr key={r.ID || r.id}>
                    ...
                    <td>
                      {Number(r.m2_residuo || r.m2Residuo || 0).toFixed(2)}
                    </td>
                    <td>
                      {Number(
                        r.porcentaje_residuo || r.porcentaje || 0
                      ).toFixed(2)}
                      %
                    </td>
                    <td>
        ...
        {error && (
          <div style={{ color: "#a30015", marginTop: 10 }}>{error}</div>
        )}
      </div>
    </div>
  );
