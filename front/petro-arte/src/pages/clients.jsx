...
import { FaUser, FaUsers, FaEdit, FaTrash, FaPlus, FaSave, FaTimes, FaCheckCircle } from "react-icons/fa";
...
const Clients = () => {
  ...
  const fetchClientes = async () => {
    ...
    try {
      ...
      let data; try { data = JSON.parse(txt); } catch { data = { clientes: [] }; }
      if (!res.ok) throw new Error(data?.mensaje || "Error al cargar clientes");
      setClientes(Array.isArray(data?.clientes) ? data.clientes : (Array.isArray(data) ? data : []));
    } catch (e) {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      const txt = await res.text();
      let data; try { data = JSON.parse(txt); } catch { data = { mensaje: txt }; }
      if (!res.ok) throw new Error(data.mensaje || data.error || "Error al crear cliente");
      closeModal();
      fetchClientes();
    } catch (err) {
  ...
  const handleEdit = async (e) => {
    ...
    try {
      ...
      const txt = await res.text();
      let data; try { data = JSON.parse(txt); } catch { data = { mensaje: txt }; }
      if (!res.ok) throw new Error(data.mensaje || data.error || "Error al editar cliente");
      closeModal();
      fetchClientes();
    } catch (err) {
      setErrorMsg(err.message);
  ...
  const handleDeactivate = async (id) => {
    ...
    try {
      ...
      const txt = await res.text();
      let data; try { data = JSON.parse(txt); } catch { data = { mensaje: txt }; }
      if (!res.ok) throw new Error(data.mensaje || data.error || "Error al desactivar cliente");
      fetchClientes();
    } catch (err) {
      setErrorMsg(err.message);
    }
  ...
  const handleReactivate = async (id) => {
    ...
    try {
      ...
      const txt = await res.text();
      let data; try { data = JSON.parse(txt); } catch { data = { mensaje: txt }; }
      if (!res.ok) throw new Error(data.mensaje || data.error || "Error al reactivar cliente");
      fetchClientes();
    } catch (err) {
      setErrorMsg(err.message);
    }
              ...
              {loading ? (
                ...
                return filtered.map((cliente) => {
                  ...
                  <tr key={cid}>
                    ...
                    <td>
                      <span className={`status-badge ${activo ? 'status-activo' : 'status-cancelado status-desactivado'}`}> 
                    ...
                    <td>
                      <div className="table-actions">
                        ...
                        {activo ? (
                          <button className="delete-btn" title="Desactivar" onClick={() => askDeactivate(cliente)}><FaTrash /></button>
                        ) : (
                          <button className="add-btn" title="Reactivar" onClick={() => askReactivate(cliente)}><FaCheckCircle /></button>
                        )}
                      </div>
                    </td>
                  </tr>
            ...
            <div className="modal-overlay">
              <div className="modal-content" style={{ width: 380, maxWidth: '90vw', color: '#222', position: 'relative' }}>
                ...
                <p style={{ marginBottom: 16 }}>
                  ¿Seguro que deseas desactivar al cliente {deactivateTarget?.nombre}? Esta acción se puede revertir reactivándolo después.
                </p>
                <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      if (deactivateTarget) {
                        await handleDeactivate(deactivateTarget.ID ?? deactivateTarget.id);
                      }
              ...
              <div className="modal-content" style={{ width: 380, maxWidth: '90vw', color: '#222', position: 'relative' }}>
                ...
                <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      if (reactivateTarget) {
                        await handleReactivate(reactivateTarget.ID ?? reactivateTarget.id);
                      }
