      ...
      beforeSave: async (ventaProducto) => {
        ...
        if (esProductoPorPiezas) {
          ...
          if (ventaProducto.tipo_medida === "piezas") {
            ...
            // Para productos por piezas: precio es por pieza
            const precio = Number(producto.precio || 0);
            if (isNaN(precio) || precio <= 0) {
              throw new Error(
                `El producto ${producto.nombre} no tiene un precio válido mayor que 0`
              );
            }
            ventaProducto.precio_unitario = precio;
            ventaProducto.subtotal = parseFloat(
              (ventaProducto.cantidad_piezas_calculada * precio).toFixed(2)
            );
          } else {
            throw new Error(
          ...
          if (ventaProducto.tipo_medida === "m2") {
            ...
            // Para productos por m²: precio es por metro cuadrado
            const precio = Number(producto.precio || 0);
            if (isNaN(precio) || precio <= 0) {
              throw new Error(
                `El producto ${producto.nombre} no tiene un precio válido mayor que 0`
            ...
            ventaProducto.precio_unitario = precio;
            ventaProducto.subtotal = parseFloat(
              (ventaProducto.cantidad_m2_calculada * precio).toFixed(2)
            );
          } else {
            throw new Error(
